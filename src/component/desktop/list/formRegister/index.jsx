...
import {Button, Col, DatePicker, Form, Input, InputNumber, Row, Select, Radio } from 'antd';
  ...
  const renderFormCustom = (index) => {
    ...
    return (
      <Row gutter={50} className={styles.formItem}>
        {data.map((el, index2) => (
          <Col className="gutter-row" span={4} sm={12} xs={24} key={`${el?.formItemProps?.name}-${index2}`}>
        ...
        <div className={'mainAppCustom'}>
          <div className={styles.formRegister}>
            <Form form={form} layout="vertical" autoComplete="off" onFinish={onFinish} initialValues={{'operator' : 'OR'}}>
              <Form.Item label={'Toán tử'} name={'operator'} className={styles.contentFormItem}>
                <Radio.Group>
